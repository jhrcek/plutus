...
import           Cardano.Node.API                           (MockNodeAPI)
...
updateContract uuid endpointName endpointPayload = do
    ...
    void $
        execStateT
            (invokeContractUpdate $ EventPayload endpointName endpointPayload)
            (oldContractState, [])
    --
...
handleContractHook ::
    ...
    => ContractHook
    -> m ()
handleContractHook (TxHook unbalancedTxs)  = handleTxHook unbalancedTxs
handleContractHook (UtxoAtHook addresses)  = handleUtxoAtHook addresses
handleContractHook (OwnPubKeyHook request) = handleOwnPubKeyHook request
...
handleTxHook unbalancedTx = do
    ...
    logInfoN $ "Signing balanced TX: " <> tshow balancedTx
    signedTx <- WAPI.signTxn balancedTx
...
handleUtxoAtHook address = do
    ...
    invokeContractUpdate $ EventPayload "utxo-at" (JSON.toJSON utxoAtAddress)
...
handleOwnPubKeyHook ::
    ...
    => OwnPubKeyRequest
    -> m ()
handleOwnPubKeyHook NotWaitingForPubKey = pure ()
handleOwnPubKeyHook WaitingForPubKey = do
    logInfoN "Handling 'own-pubkey' hook."
    key :: WAPI.PubKey <- WAPI.ownPubKey
    invokeContractUpdate $ EventPayload "own-pubkey" (JSON.toJSON key)
...
invokeContractUpdate ::
    ...
    => Payload
    -> m ()
invokeContractUpdate payload = do
...
processAllHooks =
    popHook >>= \case
        Nothing -> pure ()
        Just hook -> do
            handleContractHook hook
            sync
            processAllHooks

sync :: (MonadLogger m, MonadEventStore ChainEvent m, MockNodeAPI m) => m ()
sync = do
    (Sum oldBlocks) <- runGlobalQuery blockCount
    blocks <- NodeClient.blocksSince $ Slot oldBlocks
...
instance (MockNodeAPI m) => MockNodeAPI (StateT state m) where
    blocksSince = lift . NodeClient.blocksSince
